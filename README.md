...
KV helps you integrate your Verilog code with the KV-generated Verilog code. In the KiCad schematic, each symbol has Properties, and those properties include a list of Fields. KV looks for specific fields, and uses them to insert the code you write into the generated Verilog.
...
KV has a dependency on the pyparsing module. To install it under Windows, go to the Windows Start menu and select `KiCad 6.0 Command Prompt` (in the KiCad 6.0 folder). In the command prompt window that appears, give the command `pip install pyparsing`

On Linux, if you installed KiCad with a package manager (e.g. apt), open a regular terminal window and give the command `pip3 install pyparsing`.

Next, install the plugin itself. From Pcbnew, select `Tools->External Plugins->Open Plugin Directory`. In the displayed directory, create a folder named KiCadVerilog. Into that folder, copy the contents of [GitHub - galacticstudios/KiCadVerilog: Generate Verilog code from a KiCad netlist](https://github.com/galacticstudios/KiCadVerilog).
...
The first code KV generates is a list of **\`include** directives. You specify which files to \`include in the generated Verilog by adding fields to symbols in your KiCad schematic (see [below](#markdown-header-KiCadVerilog-Fields)). 

### Top-Level Module

The generated top-level Verilog module is given a name based on the output file name specified on the KiCadVerilog command line.
...
KV also recognizes nets that are being pulled up or pulled down. If a net is connected to a resistor, and the other end of that resistor is connected to another net that was recognized as a positive voltage, then the first net is being pulled up, and a **tri1** wire is generated in Verilog.

If a net is connected to a resistor that is connected to ground, the first net is being pulled down and KV generates it as a **tri0** wire.
...
- It does not generate module invocations or modules for bypass capacitors. Bypass capacitors are identified as parts with two pins, the word "capacitor" (case-insenistive) in their KiCad description, with one pin connected to a net with a positive voltages, and the other pin connected to a ground net.
...
Finally, KV generates a module for each part that it generated an invocation for. The port list is the name of each pin on the part (except for pins that, in KiCad, have the type "power input" or "power output"). Note that some parts don't have named pins. In those cases, the name of the port will be the pin number preceded by an underscore (e.g. "_1").
...
Next, KV inserts the Verilog code that you specified in the part's fields in the KiCad schematic editor (see [below](#markdown-header-KiCadVerilog-Fields)).

At the end of the module, KV generates an **\`undef** to undefine the macro. That way, multiple modules can have buses with the same name.
...
I welcome bug reports, bug fixes, new feature requests, new feature implementations, questions, comments, and anecdotes about how you've used KiCadVerilog.
